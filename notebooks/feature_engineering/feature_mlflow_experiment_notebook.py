...
balanced_spark_df = balanced_spark_df.dropDuplicates(["Id"])
...
# Re-apply constraints after overwrite (overwrite drops table properties)
# First make Id NOT NULL, then add primary key and enable CDF
spark.sql(
    f"ALTER TABLE {config.catalog_name}.{config.schema_name}.features_balanced ALTER COLUMN Id SET NOT NULL;"
)
spark.sql(
