...
try:
    ...
    if affected_rows_train > 0 or affected_rows_test > 0:
        ...
        spark.sql(f"""
            ...
            AND s.Id NOT IN (
                SELECT Id FROM {catalog_name}.{schema_name}.features_balanced
            )
        """)
        ...
        # Skip online feature table pipeline update (Online Tables deprecated, Lakebase not configured)
        # update_response = workspace.pipelines.start_update(pipeline_id=pipeline_id, full_refresh=False)
        logger.info("Skipping online feature table pipeline update (not configured).")
