...
try:
    ...
    config = serving_endpoint.config or serving_endpoint.pending_config
    if config is None:
        raise ValueError(f"Endpoint '{serving_endpoint_name}' has no config or pending_config. State: {serving_endpoint.state}")
    if config.served_entities:
        model_name = config.served_entities[0].entity_name
        model_version = config.served_entities[0].entity_version
    else:
        model_name = config.served_models[0].model_name
        model_version = config.served_models[0].model_version
    ...
    auc_new = roc_auc_score(df_pd["Default"], df_pd["prediction_new"])

    # Calculate AUC for the old model
    auc_old = roc_auc_score(df_pd["Default"], df_pd["prediction_old"])
